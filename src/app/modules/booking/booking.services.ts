...
import { BAD_REQUEST, CONFLICT, NOT_FOUND, SERVICE_UNAVAILABLE } from "http-status";
...
const createBookingIntoDB = async (bookingData: TBookingData) => {
    const { serviceId, slotId, customer, ...rest } = bookingData;
    ...
    try {
        ...
        if (!newBooking) {
            throw new AppError(SERVICE_UNAVAILABLE, 'Unable to create a new booking at this time.');
        }

        // Update slot status to booked
        const updateSlotToBooked = await Slot.findByIdAndUpdate(slotId, { isBooked: 'booked' }, { session, new: true });
        if (!updateSlotToBooked) {
            throw new AppError(SERVICE_UNAVAILABLE, 'Unable to update the slot status to booked.');
        }
